...
NOISE_CLIP = 0.5
CHALLENGE_AMOUNT = 5
TRAINING_MEAN_REWARD_MAXLEN = 30
TESTING_MEAN_REWARD_MAXLEN = 30
...
TRAIN_BOOL = False
VERSION_CONTROL = "Demo"  # Does not matter if TRAIN_BOOL is set to False (New version is only created when training)

# SET TEST BOOL, SET VERSION NUMBER TO LOAD IN TEST (MODEL)
TEST_BOOL = True
MODEL_VERSION_LOAD = "Newest-10"  # Does not matter if TEST_BOOL is set to False (Loading a model is to test it)
# USED FOR LOADING IN PEAKS FROM SPECIFIC MODEL (MODEL_VERSION_LOAD IS THE MODEL YOU LOAD PEAK FROM)
IS_PEAK = False
EP_NUM = 1396
SCORE = "262.00"
...
class TD3:
    ...
    def train(self):
        ...
        episode_reward = 0
        episode_steps = 0
        mean_reward_lastX = deque(maxlen=TRAINING_MEAN_REWARD_MAXLEN)
        mean_reward_history = []
        highScore = 0
        meanValue = 0
        ...
        peakPath = "OwnTake/loadModels/bipedalOwnTake{}/Peaks".format(VERSION_CONTROL)
        ...
        for steps_done in range(TOTAL_STEPS):
            ...
            if done:
                ...
                if len(mean_reward_lastX) == TRAINING_MEAN_REWARD_MAXLEN:

                    meanValue = sum(mean_reward_lastX) / len(mean_reward_lastX)
                    mean_reward_history.append(meanValue)

                mean_reward_lastX.append(episode_reward)

                # Save highScores over 200
                if (meanValue >= 200) and (meanValue > highScore):
                    highScore = meanValue

                    torch.save(
                        self.challenge_predict_actor.state_dict(),
                        "OwnTake/loadModels/bipedalOwnTake{}/Peaks/challPredEp{}-Score{:.2f}.pth".format(
                            VERSION_CONTROL, episodes_done, highScore
                        ),
                    )
        ...
        plt.xlabel("Episode + {}".format(TRAINING_MEAN_REWARD_MAXLEN))
    ...
    def test(self, episodes):
        ...
        if IS_PEAK:
            self.challenge_predict_actor.load_state_dict(
                torch.load(
                    "OwnTake/loadModels/bipedalOwnTake{}/Peaks/challPredEp{}-Score{}.pth".format(
                        MODEL_VERSION_LOAD, EP_NUM, SCORE
                    )
                )
            )
        else:
        ...
        reward_history = []
        mean_reward_history = []
        mean_reward_lastX = deque(maxlen=TESTING_MEAN_REWARD_MAXLEN)
        current_actor_history = [0] * CHALLENGE_AMOUNT
        ...
        for i in range(episodes):
            ...
            if len(mean_reward_lastX) == TESTING_MEAN_REWARD_MAXLEN:
        ...
        if IS_PEAK:
            newPath = "OwnTake/testResults/OwnTake{}-Peak02{}-{}".format(
                MODEL_VERSION_LOAD, EP_NUM, SCORE
            )
        else:
            newPath = "OwnTake/testResults/OwnTake{}".format(MODEL_VERSION_LOAD)
        if not os.path.exists(newPath):
            os.makedirs(newPath)
        ...
        # Save Graph
        plt.savefig("{}/RewardGraph.png".format(newPath))
        ...
        plt.plot(mean_reward_history)
        plt.title("Mean Reward History")
        plt.xlabel("Episode + {}".format(TESTING_MEAN_REWARD_MAXLEN))
        plt.ylabel("Mean Reward")

        # Save Graph
        plt.savefig("{}/MeanRewardGraph.png".format(newPath))
        ...
        # Save Graph
        plt.savefig("{}/ActorGraph.png".format(newPath))

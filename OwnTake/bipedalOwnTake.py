...
class TD3:
    def __init__(self):
        ...
        ### MAIN ACTORS
        self.actorList = []
        self.actor_targetList = []
        self.actor_optimizerList = []
        self.criticsList = []
        self.critics_targetList = []
        self.critics_optimizerList = []
        ...
        for x in range(CHALLENGE_AMOUNT):
            ...
            self.actor.load_state_dict(torch.load("OwnTake/loadModels/walkModel.pth"))
            ...
            self.actor_targetList.append(self.actor_target)

            # Set actor optimizer
            self.actor_optimizer = torch.optim.Adam(self.actor.parameters(), lr=LR)
            self.actor_optimizerList.append(self.actor_optimizer)

            # Critics Setup
            self.critics = Critics(self.state_size, self.action_size).to(DEVICE)
            self.criticsList.append(self.critics)
            ...
            self.critics_targetList.append(self.critics_target)

            # Set critic optimizer
            self.critics_optimizer = torch.optim.Adam(self.critics.parameters(), lr=LR)
            self.critics_optimizerList.append(self.critics_optimizer)
    ...
    def train(self):
        ...
        for steps_done in range(TOTAL_STEPS):
            ...
            if done:
                ...
                if episode_reward >= 200:
                    ...
                    # Save actors when peak
                    for index, actor in enumerate(self.actorList):
                        torch.save(
                            actor.state_dict(),
                            "OwnTake/loadModels/bipedalOwnTake01/Peaks/Actors/{}actor{}.pth".format(
                                peaks_counter, index
                            ),
                        )

                    # Save predicter when peak
                    torch.save(
                        self.challenge_predict_actor.state_dict(),
                        "OwnTake/loadModels/bipedalOwnTake01/Peaks/Predicters/{}predicter01.pth".format(
                            peaks_counter
                        ),
                    )
        ...
        for index, actor in enumerate(self.actorList):
            torch.save(
                actor.state_dict(),
                "OwnTake/loadModels/bipedalOwnTake01/Actors/actor{}.pth".format(index),
            )

        # Save Predict Network
        torch.save(
            self.challenge_predict_actor.state_dict(),
            "OwnTake/loadModels/bipedalOwnTake01/Predicter/challengePredicter01.pth",
        )
        ...
        # Plot the Reward History
        plt.plot(current_actor_history)
    ...
    def optimize(self, current_iteration, current_actor):
        ...
        with torch.no_grad():
            ...
            next_action = (
                self.actor_targetList[current_actor](next_state) + noise
            ).clamp(-self.max_action_t, self.max_action_t)
            ...
            target_Q1, target_Q2 = self.critics_targetList[current_actor](
        ...
        current_Q1, current_Q2 = self.criticsList[current_actor](state, action)
        ...
        # Optimize the critic
        self.critics_optimizerList[current_actor].zero_grad()
        critics_loss.backward()
        self.critics_optimizerList[current_actor].step()
        ...
        if current_iteration % POLICY_FREQ == 0:
            ...
            # Optimize the actor
            self.actor_optimizerList[current_actor].zero_grad()
            actor_loss.backward()
            self.actor_optimizerList[current_actor].step()
            ...
            for param, target_param in zip(
                self.criticsList[current_actor].parameters(),
                self.critics_targetList[current_actor].parameters(),
            ):
            ...
            # actor
            for param, target_param in zip(
                self.actorList[current_actor].parameters(),
                self.actor_targetList[current_actor].parameters(),
            ):
                target_param.data.copy_(
                    TAU * param.data + (1 - TAU) * target_param.data
                )
